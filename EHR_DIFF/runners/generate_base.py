...
def postprocess_columns(X, binary_cols=None, continuous_cols=None):
    ...
    #binary col 
    #for all binary 
    bin_mask = (np.array(binary_cols, dtype=bool) if (np.asarray(binary_cols).dtype == bool and len(binary_cols) == D) else np.zeros(D, dtype=bool))
    #given binary cols 
    ...
    #continuous col 
    cont_mask = (np.array(continuous_cols, dtype=bool) if (np.asarray(continuous_cols).dtype == bool and len(continuous_cols) == D) else np.zeros(D, dtype=bool))
    #set mask for continuous_cols 
    ...
    #clip + round as they do 
    if bin_mask.any():
        X[:, bin_mask] = np.rint(np.clip(X[:, bin_mask], 0.0, 1.0))
    #clip (normalized) as they do 
    if cont_mask.any():
...
def evaluation(config, workdir):
    ...
    if config.test.labels is not None:
        ...
        if config.setup.global_rank == 0:
            ...
            all_x_across_all_gpus = postprocess_columns(
        ...
        if config.setup.global_rank == 0:
            ...
            BINARY_COL = getattr(config.data, 'binary_cols', None)       
            CONTINUOUS_COL = getattr(config.data, 'continuous_cols', None) 
            all_x_across_all_gpus = postprocess_columns(all_x_across_all_gpus, binary_cols=BINARY_COL, continuous_cols=CONTINUOUS_COL)
            np.save(os.path.join(sample_dir, 'all_x'), all_x_across_all_gpus)

...
from evaluation.evaluate_mimic import (train_and_test_classification)
import torch, torch.nn as nn, torch.nn.functional as F
...
def evaluate_model(df_train_norm, df_hold_norm, df_real_norm, df_syn_norm, OUT_DIR, seed, result_csv, batch, time_dim, learning_rate, trial, elapsed_time): 
    #-----------------UTILITY-----------------#
    ...
    with open(f"{OUT_DIR}/eval.txt", "a") as f:
        ...
        f.write(f"BATCH: {batch}, TIME_DIM: {time_dim}, LR: {learning_rate}, elapsed_time: {elapsed_time}\n")
    log_result(result_csv, trial, seed, batch, time_dim, learning_rate, s2h_auc, s2h_prauc, s2h_acc, r2s_auc, r2s_prauc, r2s_acc, elapsed_time)
...
def objective(trial:optuna.Trial, study_name, data_path): 
    #define parameters 
    batch = trial.suggest_int("batch_size", 256, 4096, step=256)
    lr = trial.suggest_float("learning_rate", 3e-5, 3e-3, log=True)
    time_dim = trial.suggest_categorical("time_dim", [320, 384, 448])
    ...
    npy_path = Path(data_path) / "min_max_log.npy"
    ...
    train_array_npy = Path(data_path) / "normalized_training_data.array.npy"
    ...
    train_overrides = {"data": {"path": str(train_array_npy)}, "train": {"batch_size": batch}, "model": {"network": {"time_dim": time_dim, "unit_dims": [1024, time_dim, time_dim, time_dim, 1024]}}, 
        "optim": {"params": {"lr": lr}}, 
    }
    sample_overrides = {"model": {"ckpt": str(ckpt_path), "network": {"time_dim": time_dim, "unit_dims": [1024, time_dim, time_dim, time_dim, 1024], }}}
    ...
    try:
        s2h_auc, r2s_auc = evaluate_model(df_train_norm, df_hold_norm, df_real_norm, df_syn_norm, trial_dir, seed, result_csv, batch, time_dim, lr, str(trial.number), elapsed_time)          
...
def main(): 
    ...
    study.optimize(lambda trial: objective(trial, study_name, data_path), n_trials = 50)

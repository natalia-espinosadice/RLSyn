...
DATA_ROOT = REPO_ROOT.parent / "MIMIC_DATA" 
...
def evaluate_model_MIMIC(df_train, df_real, df_syn, df_train_norm, df_hold_norm, df_real_norm, df_syn_norm, elapsed_time, OUT_DIR, SEED, RESULT_CSV, RUN_NAME): 
    ...
    log_result_RL_MIMIC(RESULT_CSV, RUN_NAME, None, None, SEED, cwc, ad2d, continuous_w_d, latent_cluster_analysis, mca_dist, mca_tvd_dist, combined_clinical_violations, s2h_auc, s2h_prauc, s2h_acc, r2s_auc, r2s_prauc, r2s_acc, r2r_auc, r2r_prauc, r2r_acc, mem_aucs, elapsed_time)
    return s2h_auc, r2s_auc 


def train(base_dir, seed, epochs, batch_size, num_channels, random_dim ):

    data_path = DATA_ROOT / f"seed{seed}"
    ...
    train_csv = DATA_ROOT / f"seed{seed}" / f"normalized_training_data_{seed}.csv"
    test_csv  = DATA_ROOT / f"seed{seed}" / f"normalized_testing_data_{seed}.csv"
    raw_csv = train_csv  # CTAB-GAN reads from a csv path directly
    start_time = time.time()
    synthesizer = CTABGAN(
        raw_csv_path=str(raw_csv),
        test_ratio=0.001,                  # already using the train split
        ...
        integer_columns=[],
        problem_type={"Classification": "DIE_1y"}
    )
    ...
    synthesizer.synthesizer.sides = [4, 8, 16, 24, 32, 64, 128]
    ...
    evaluate_model_MIMIC(df_train, df_real, df_syn, df_train_norm, df_hold_norm, df_real_norm, df_syn_norm, elapsed_time, trial_dir, seed, results_csv, f"ctabgan_seed{seed}")    
...
def main():
    args = parse_args()
    seed = int(args["seed"])
    base_dir = REPO_ROOT / "CTAB_GAN_MIMIC" 

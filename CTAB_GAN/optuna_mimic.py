...
def log_result(RESULTS_CSV, tag, seed, epochs, batch_size, num_channels, random_dim, s2h_auc, s2h_prauc, s2h_acc, r2s_auc, r2s_prauc, r2s_acc, elapsed_time):
    p = Path(RESULTS_CSV)
    p.parent.mkdir(parents=True, exist_ok=True)
    row = {"tag": tag, "seed" : seed,
        "epochs": epochs, "batch_size": batch_size, "num_channels": num_channels, "random_dim": random_dim, 
...
def evaluate_model(df_train_norm, df_hold_norm, df_real_norm, df_syn_norm, OUT_DIR, seed, result_csv, epochs, batch_size, num_channels, random_dim, trial, elapsed_time): 
    #-----------------UTILITY-----------------#
    ...
    with open(f"{OUT_DIR}/eval.txt", "a") as f:
        ...
        f.write(f"epochs: {epochs}, batchsize: {batch_size}, num_channels: {num_channels}, random_dim: {random_dim}\n")
    log_result(result_csv, trial, seed, epochs, batch_size, num_channels, random_dim, s2h_auc, s2h_prauc, s2h_acc, r2s_auc, r2s_prauc, r2s_acc, elapsed_time)
...
def objective(trial: optuna.Trial, study_name: str):
    ...
    try:
        s2h_auc, r2s_auc = evaluate_model(
            df_train_norm, df_hold_norm, df_real_norm, df_syn_norm,
